...
__global__ void Gelu_Half_kernel(float *input, float *output, size_t size, int col) {
...
int DLGpuGeluHalf(const DLArrayHandle input, DLArrayHandle output,
    ...
    int col = input->shape[input->ndim - 1];
    dim3 blocks;
    dim3 threads;
    float *input_data = (float *)input->data;
    float *output_data = (float *)output->data;
    ...
    if (stream_handle)
        Gelu_Half_kernel<<<blocks, threads, 0,
                      *(cudaStream_t *)stream_handle->handle>>>(
            input_data, output_data, size, col);
    else
        Gelu_Half_kernel<<<blocks, threads>>>(input_data, output_data, size, col);
    return 0;
}