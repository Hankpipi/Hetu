...
def scatter_for_linear(in_arr, out_arr, index, activation_mode=0, merge_rate=1.0, stream=None):
    assert isinstance(in_arr, _nd.NDArray)
    assert isinstance(out_arr, _nd.NDArray)
    assert isinstance(index, _nd.NDArray)
    ...
    _LIB.DLGpuScatterForLinear(
            in_arr.handle, out_arr.handle, index.handle,
            activation_mode, ctypes.c_float(merge_rate), stream.handle if stream else None)
