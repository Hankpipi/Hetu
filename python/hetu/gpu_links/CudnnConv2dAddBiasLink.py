...
def CuDNN_conv2d_with_bias(in_arr_x, in_arr_f, bias, out_arr, padding=(0, 0), stride=(1, 1), stream=None, cache_need_release=0):
...
def CuDNN_conv2d_with_bias_sparse(in_arr_x, in_arr_f, bias, out_arr, 
                    ...
                    activation_mode=0, scale=None, shift=None, stream=None, cache_need_release=0):
    assert isinstance(in_arr_x, _nd.NDArray)
    assert isinstance(in_arr_f, _nd.NDArray)
    assert isinstance(bias, _nd.NDArray)
